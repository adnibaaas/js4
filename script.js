...
There is a new gymnastics discipline, which works
differently. Each team competes 3 times, and then the average of the 3 scores is calculated (so one average score per team). 
...
let checkWinner = function(koalasAverage,dolphinsAverage){
    if(dolphinsAverage < 2* koalasAverage){
        console.log(`Dolphines are the winner with score ${dolphinsAverage}`)
    } else{
        console.log(`Koalas are th winner with score ${koalasAverage}`)
    }
}
